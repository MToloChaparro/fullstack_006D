...
public class InventarioController {
    ...
    // GET: http://localhost:8081/api/inventario/{id}
    @GetMapping("/{id}")
    ...
    // GET: http://localhost:8081/api/inventario/valor-total
    @GetMapping("/valor-total")
    public ResponseEntity<?> obtenerCalculoValorTotal() {
        try {
            double valorTotal = inventarioService.calcularValorTotalInventario();
            // Lo devolvemos estructurado dentro de un JSON limpio clave-valor
            return ResponseEntity.ok(Map.of(
                    "descripcion", "Valor total acumulado de insumos en inventario",
                    "total", valorTotal
            ));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
...
// DELETE: http://localhost:8081/api/inventario/{id}
    @DeleteMapping("/{id}")
    ...
    // PUT: http://localhost:8081/api/inventario/{id}
    @PutMapping("/{id}")
